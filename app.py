...
# Create tabs
tab_names = ["Job Description Parser", "Query your documents with different embedding models"]
...
with tabs[0]:
    ...
    if st.session_state.prompt_data:
        ...
        if st.button("Use this prompt for candidate search", key="use_prompt_button"):
            ...
            # Signal to switch to tab 2 on next rerun
            st.session_state.switch_to_tab2 = True
            # Display a message about switching
            st.success("Prompt saved! Switching to search tab...")
...
with tabs[1]:
    ...
    # Submit button for retrieval specific to this tab
    search_query_submitted = st.button("Search", type="primary", use_container_width=True, key="search_button_tab2")

    # Use a tab-specific variable for tracking query execution
    if "tab2_query_executed" not in st.session_state:
        st.session_state.tab2_query_executed = False

    # Store tab2-specific query and status
    if search_query_submitted and st.session_state.query_input:
        # Make sure we stay on this tab
        st.session_state.active_tab_index = 1
        st.session_state.switch_to_tab2 = False
        # Store the query for processing
        st.session_state.tab2_query = st.session_state.query_input
        st.session_state.tab2_query_executed = True
    
    # Only execute the search if the tab2 query has been submitted
    if st.session_state.get('tab2_query_executed', False):
        query = st.session_state.tab2_query
